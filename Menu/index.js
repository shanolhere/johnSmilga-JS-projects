...
const displayButtonList = () => {
  let buttonList =["All", ...new Set(menu.map((item) => {
    return item.category;
  }))]
  //console.log(buttonList)

  let categoryBtns = buttonList.map((item) => {
  ...
  btns.forEach(btn => {
    btn.addEventListener('click', () => {
      const categoryClicked =  btn.innerHTML;
      const categoryListMenu = menu.filter((item) => {
        if(categoryClicked===item.category){
          return item;
        }
      })
      if(categoryClicked==="All"){
        ...
        displayMenuItems(categoryListMenu)
      }
    })
  })
}
